...
if ERRORLEVEL 1 (
  ...
  if ERRORLEVEL 1 (
    ...
    echo %ADBPATH% %ARGS% shell -x "/data/local/tmp/strserv -i &" >> tmp\_win.bat
REM    echo /data/local/tmp/strserv -i ^& > tmp\_script.sh
  )
) else (
    echo %ADBPATH% %ARGS% shell "/data/local/tmp/strserv -c &" >> tmp\_win.bat
REM  echo /data/local/tmp/strserv -c ^& > tmp\_script.sh
)
...
REM ## start
REM echo title Close this window after Server Started. > tmp\_win.bat
